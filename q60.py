...
extra space complexity, since this is what you will be asked to accomplish
in an interview.
...
def kthSmallestInBST(t, k):
    ...
    def treetraverse(t, k, current):

        if t is not None:
            treetraverse(t.left, k, current)
            current[0] += 1
            # print(t.value, k, current[0])
            if k == current[0]:
                current[1] = t.value
            treetraverse(t.right, k, current)

    # 0th element will contain the item number
    # 1st item will contain the value
    current = [0] * 2

    treetraverse(t, k, current)
